...
// ========== 压缩和导出 - 原生 ZipPlugin 实现 ==========

/**
 * 将会话文件夹打包为 zip 文件（使用原生 ZipPlugin）
 * @param {string} sessionFolderName - 会话文件夹名
 * @param {string} [zipFileName] - 输出 zip 文件名（不含扩展名）
 * @returns {Promise<{uri:string,path:string,relativePath:string,cached:boolean}>}
...
export async function zipSessionToFile(sessionFolderName, zipFileName) {
  ...
  try {
    // 使用 Capacitor 原生插件注册表调用 ZipPlugin
    ...
    logger.error('原生ZipPlugin打包失败', { error: e.message })
    throw new FilePathError(ErrorCodes.FILESYSTEM_ERROR, `打包失败: ${e.message}`)
  }
}
